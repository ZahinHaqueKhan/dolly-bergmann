...
export const metadata = {
  title: 'Order detail — Wholesale Portal',
...
export default async function MyOrderDetailPage({
  ...
  let order
  try {
    order = await getMyOrder(Number(id))
  } catch {
    notFound()
  }
  ...
  return (
    <>
          ...
          <tbody>
            {order.line_items.map((li) => (
              <tr key={li.id} className="border-b border-stone-100">
                ...
                <td className="py-2 text-right text-stone-700">{li.quantity}</td>
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-6">
        <h3 className="font-serif text-lg text-stone-800 mb-2">Payment</h3>
        ...
        {order.pdf_path && (
          <p className="text-sm text-stone-500 mt-3">
            <a
              href={`/api/admin/wholesale/quotes/${order.quote_id}/pdf`}
              className="text-rose-500 hover:underline"
              target="_blank"
              rel="noreferrer"
            >
              Download original quote PDF
            </a>
          </p>
        )}
      </div>
