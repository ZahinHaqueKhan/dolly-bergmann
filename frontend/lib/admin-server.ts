// lib/server-auth.ts
//
// Server-side helper for admin pages. Reads the httpOnly auth cookies
// and calls the FastAPI backend. Used by the /admin/* server components
// for the auth-gate check, dashboard fetch, products list, etc.
...
export async function adminPost<T>(path: string, body: unknown): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new ServerAuthError(
      `POST ${path} -> ${res.status}: ${text || '(no body)'}`,
      res.status,
    )
  }
  return res.json() as Promise<T>
...
export async function adminPut<T>(path: string, body: unknown): Promise<T> {
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => '')
    throw new ServerAuthError(
      `PUT ${path} -> ${res.status}: ${text || '(no body)'}`,
      res.status,
    )
  }
  return res.json() as Promise<T>
...
export async function adminDelete(path: string): Promise<void> {
  ...
  if (!res.ok && res.status !== 204) {
    const text = await res.text().catch(() => '')
    throw new ServerAuthError(
      `DELETE ${path} -> ${res.status}: ${text || '(no body)'}`,
      res.status,
    )
  }
}
